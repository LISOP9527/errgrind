...
class TeachWorkflow:
    ...
    def start_or_resume(
        ...
        *,
        ...
        if error.status == "pending-grill" or not error.grilling_conversation:
            raise InvalidWorkflowState("请先进行 grill 审讯分析")
        ...
        return ConversationResult(self._error(error_id), messages, None, resumed)
    ...
    def finish(self, error_id: int) -> ConversationResult:
        ...
        return ConversationResult(self._error(error_id), messages, None, bool(messages))
    ...
    def _respond(self, error_id, messages, *, resumed, before_model_call):
        ...
        self._save(error_id, messages)
        return ConversationResult(self._error(error_id), messages, response, resumed)
