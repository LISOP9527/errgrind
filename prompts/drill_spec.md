...
1. 只选择一个最值得训练的 Error Pattern，并在 `source_error_number` 中填写对应的 `[Error N]` 编号。
2. Pattern 应来自 Grill 摘要，描述可迁移的思维机制，而不是某个知识点或一次计算失误。
3. 为下一阶段设计一道全新的数学题。不要复述原题，也不要只改数字、字母或背景。
4. `success_signal` 描述判断学生是否克服该 Pattern 时，应从其答案与思路中观察到的具体思考行为；不能只写“答对”或“理解了”。
5. `task_type` 只能是：`calculate`、`simplify`、`solve`、`prove`、`classify`、`construct`、`optimize`、`explain`、`determine_truth`。
6. 难度字段使用 1–5 的整数，并尽量接近所选 Error。
...
{{
  ...
  "target_pattern": {{
    ...
    "success_signal": "判断学生克服该 Pattern 时，应观察到的思考行为"
  }},
  "new_problem": {{
    "domain": "新题采用的数学领域",
    "task_type": "calculate",
