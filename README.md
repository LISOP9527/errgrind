...
可在 WebUI 的 **Settings** 中调整 Provider、Model、Codex effort、API Key、OpenCode 地址、Drill 上下文条数和 Grill 最大轮数。设置保存后，当前 Web 进程的后续模型请求立即使用新值；若通过 CLI 修改配置，需重启 Web 进程。Codex 的 OAuth 登录仍需在终端运行 `.venv/bin/errgrind` 完成。
...
- **Grill / Teach**：在同一个 Error 对话工作区中开始、恢复、暂停或继续讨论；完成的 Grill 只读，Teach 保存后仍可继续。两者都可以用同一个 `+ 图片` 入口发送图片；图片随用户消息持久化，失败后刷新可恢复，页面只显示紧凑附件标记。
